...
float _BorderWaveFrequency; // Added property for border wave frequency
float _BorderWaveAmplitude; // Added property for border wave amplitude
...
fixed4 frag(v2f i) : SV_Target
{
    ...
    borderColor.rgb *= 0.7; // Adjust this value to control darkness (e.g., 0.7 for 70% brightness)

                // Calculate outline distance and mask
    float outlineDist = borderDist - _OutlineThickness; // Outline is slightly outside the border
    ...
    fixed4 outlineColor = borderColor * 0.5; // Make the outline even darker
